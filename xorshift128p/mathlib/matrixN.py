...
def identity_matN(N):
    return [gmpy2.mpz(1<<i) for i in range(N)] 

def rand_perm_matN(N):
    R = identity_matN(N)
    random.shuffle(R)
    return R
...
def mul_matN(M, N):
    """
        mul_mat128(): Multiply 2 axb, bxc matrices.
    """
    M_rows = len(M)
    N_rows = len(N)

    M = deepcopy(M)
    R = [gmpy2.mpz(0)] * M_rows
...
def kernel_left_basis(M, nrows, ncols):
    M = deepcopy(M)

    # Augment it with an nrows x nrows identity matrix on the right
    for i_row in range(nrows):
        M[i_row] = gmpy2.bit_set(M[i_row], ncols + i_row)    

    # RREF it!
    M = rref(M, nrows, ncols + nrows)

    # Get the left side of the augmented matrix
    K = []
    for i_row in range(nrows):
        row_augR, row_augL = gmpy2.t_divmod_2exp(M[i_row], ncols)
        if row_augL == 0 and row_augR != 0:
            K.append(row_augR)
